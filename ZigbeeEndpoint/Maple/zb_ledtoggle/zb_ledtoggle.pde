...
//
//
// This is a simple LeafLabs Maple demo to show a Syntro app controlling
// the Maple's BOARD_LED_PIN remotely using a Zigbee radio.
//
...
//
// The protocol is very simple, write two bytes and the Maple will
...
//
// Write one byte to the Maple and that will be interpreted as a read.
// The Maple will respond back with a two byte write. The first byte
...
#ifdef USBSERIAL_DEBUG        
    dumpTxBuff(txBuff, len);
#endif

    delay(100);
    
    Serial1.write((void *)txBuff, len);
}
...
#ifdef USBSERIAL_DEBUG
void dumpTxBuff(unsigned char *txBuff, int len)
{
    int i;
